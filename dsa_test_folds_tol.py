...
train_images, val_images, train_masks, val_masks = train_test_split(
    ...
    train_masks,
    test_size=0.2,
    random_state=42  # Must match training code!
)
...
for img_idx, (val_image_padded, val_mask_padded, original_mask) in enumerate(zip(val_images_padded, val_masks_padded, gt_masks)):
    ...
    rand_ele = np.random.choice(val_image_padded[40:-40, 40:-40].flatten(), num, replace=False)
    indices = np.argwhere(np.isin(val_image_padded[40:-40, 40:-40], rand_ele)).tolist()
    ...
    # After creating flat_list_ind and flat_list_pred
