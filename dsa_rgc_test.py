...
model.load_state_dict(torch.load("/home/akaur101/data/f25/dsa_rgc_two_images_bestmodel_dict", map_location=device))
model.eval()
...
with open(os.path.join(save_dir, "all_pred_list_trained_on_two"), 'w') as fp:
    for item in all_pred:
        # write each item on a new line
        fp.write("%s\n" % item)
    print('Done with all_pred')
    
with open(os.path.join(save_dir, "all_ind_list_trained_on_two"), 'w') as fp:
...
plt.savefig(os.path.join(save_dir, "new_recon_mask_trained_on_two.png"))
plt.close()
